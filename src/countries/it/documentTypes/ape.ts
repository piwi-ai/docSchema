...
import { num, text, enumField, objectSchema, ref, datePattern } from '../helpers.js';
import { ReferenceType } from '../../../constants.js';

const CLASSE_ENERGETICA_VALUES = ['A4', 'A3', 'A2', 'A1', 'B', 'C', 'D', 'E', 'F', 'G'];

const DESTINAZIONE_USO_DESC =
    "Destinazione d'uso dell'immobile (es. residenziale, ufficio, commerciale, industriale, agricolo, magazzino, autorimessa)";
...
export const ape: DocumentTypeDef = {
    ...
    references: [
        ref(
            'ENEA — Attestato di Prestazione Energetica',
            'https://www.efficienzaenergetica.enea.it/',
            ReferenceType.DOCUMENTATION,
        ),
        ref(
            "D.Lgs. 192/2005 — Rendimento energetico nell'edilizia",
            'https://www.normattiva.it/uri-res/N2Ls?urn:nir:stato:decreto.legislativo:2005-08-19;192',
            ReferenceType.REGULATION,
        ),
        ref(
            'SIAPE — Sistema Informativo APE',
            'https://siape.enea.it/',
            ReferenceType.DOCUMENTATION,
        ),
    ],
    jsonSchema: objectSchema(
        {
            classeEnergetica: enumField(
                "Classe energetica dell'immobile",
                CLASSE_ENERGETICA_VALUES,
            ),
